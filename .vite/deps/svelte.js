...
function setContext(key, context) {
	...
	if (async_mode_flag) {
		...
		if (!(!active_reaction && (flags & BRANCH_EFFECT) !== 0 && !component_context.i)) set_context_after_init();
	}
...
function handle_error(error) {
	...
	if (true_default && error instanceof Error && !adjustments.has(error)) adjustments.set(error, get_adjustments(error, effect));
	if ((effect.f & REACTION_RAN) === 0 && (effect.f & EFFECT) === 0) {
		if (true_default && !effect.parent && error instanceof Error) apply_adjustments(error);
		throw error;
	}
	...
	*/
	#traverse_effect_tree(root, effects, render_effects) {
		...
		while (effect !== null) {
			...
			if (!(is_branch && (flags & CLEAN) !== 0 || (flags & INERT) !== 0 || this.#skipped_branches.has(effect)) && effect.fn !== null) {
				if (is_branch) effect.f ^= CLEAN;
				else if ((flags & EFFECT) !== 0) effects.push(effect);
				else if (async_mode_flag && (flags & (RENDER_EFFECT | MANAGED_EFFECT)) !== 0) render_effects.push(effect);
...
function flush_effects() {
		...
		while (queued_root_effects.length > 0) {
			...
			if (flush_count++ > 1e3) {
				if (true_default) {
					...
					for (const source of batch.current.keys()) for (const [stack, update] of source.updated ?? []) {
			...
			old_values.clear();
			if (true_default) for (const source of batch.current.keys())
 /** @type {Set<Source>} */ source_stacks.add(source);
		...
		is_flushing = false;
		last_scheduled_effect = null;
		if (true_default) for (const source of source_stacks) source.updated = null;
	}
}
...
function schedule_effect(signal) {
	...
	while (effect.parent !== null) {
		effect = effect.parent;
		var flags = effect.f;
		if (is_flushing && effect === active_effect && (flags & BLOCK_EFFECT) !== 0 && (flags & HEAD_EFFECT) === 0 && (flags & REACTION_RAN) !== 0) return;
...
function fork(fn) {
	...
	for (source of batch.current.keys()) if ((source.f & DERIVED) !== 0) set_signal_status(source, DIRTY);
	return {
		commit: async () => {
			if (committed) {
				await settled;
...
function set(source, value, should_proxy = false) {
	if (active_reaction !== null && (!untracking || (active_reaction.f & EAGER_EFFECT) !== 0) && is_runes() && (active_reaction.f & (DERIVED | BLOCK_EFFECT | ASYNC | EAGER_EFFECT)) !== 0 && (current_sources === null || !includes.call(current_sources, source))) state_unsafe_mutation();
	let new_value = should_proxy ? proxy(value) : value;
	if (true_default) tag_proxy(new_value, source.label);
...
function internal_set(source, value) {
	if (!source.equals(value)) {
		...
		if (is_runes() && active_effect !== null && (active_effect.f & CLEAN) !== 0 && (active_effect.f & (BRANCH_EFFECT | ROOT_EFFECT)) === 0) if (untracked_writes === null) set_untracked_writes([source]);
...
function mark_reactions(signal, status) {
	...
	for (var i = 0; i < length; i++) {
		...
		if (true_default && (flags & EAGER_EFFECT) !== 0) {
		...
		if ((flags & DERIVED) !== 0) {
			...
			if ((flags & BLOCK_EFFECT) !== 0 && eager_block_effects !== null) eager_block_effects.add(reaction);
			schedule_effect(reaction);
		}
	}
}
	...
	return new Proxy(value, {
		defineProperty(_, prop, descriptor) {
			...
			if (s === void 0) with_parent(() => {
				...
				sources.set(prop, s);
				if (true_default && typeof prop === "string") tag(s, get_label(path, prop));
				return s;
			});
...
function create_effect(type, fn, sync) {
	...
	if (true_default) effect.component_function = dev_current_component_function;
	if (sync) try {
		update_effect(effect);
	} catch (e) {
...
function block(fn, flags = 0) {
	...
	if (true_default) effect.dev_stack = dev_stack;
	return effect;
}
/**
* @param {(() => void)} fn
...
function destroy_effect(effect, remove_dom = true) {
	...
	if (true_default) effect.component_function = null;
...
function schedule_possible_effect_self_invalidation(signal, effect, root = true) {
	...
	for (var i = 0; i < reactions.length; i++) {
		var reaction = reactions[i];
		if ((reaction.f & DERIVED) !== 0) schedule_possible_effect_self_invalidation(reaction, effect, false);
...
function append(anchor, dom) {
	if (hydrating) {
		var effect = active_effect;
		if ((effect.f & REACTION_RAN) === 0 || effect.nodes.end === null) effect.nodes.end = hydrate_node;
		hydrate_next();
		return;
	}
...
function createRawSnippet(fn) {
	return (anchor, ...params) => {
		...
		if (hydrating) {
			...
			if (true_default && (/* @__PURE__ */ get_next_sibling(element) !== null || element.nodeType !== ELEMENT_NODE)) invalid_raw_snippet_render();
...
function hydratable(key, fn) {
	...
	if (hydrating) {
		...
		if (true_default) hydratable_missing_but_required(key);
